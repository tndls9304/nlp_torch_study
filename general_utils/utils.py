...
def pickle_reader(path):
    ...
    try:
        ...
        print("input is not type of dictionary: input type = {} read from {}".format(type(data), path))
    return data


def pickle_writer(path, target):
    with open(path, 'wb') as f:
        pickle.dump(target, f)
    try:
        print('size {} dictionary is written in {}'.format(len(target), path))
    except TypeError:
        print("input is not type of dictionary: input type = {} written in {}".format(type(target), path))
