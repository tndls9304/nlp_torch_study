...
class S2STrainer:
    def __init__(self, config, device):
        ...
        self.dataset = Seq2SeqDataset(self.config, tokenize_fr_nltk, tokenize_en_nltk, device)
    ...
    def train_epoch(self, epoch=0, orig_path=None, pred_path=None):
        ...
        for i, batch in enumerate(tqdm(self.dataset.train_iterator)):
            ...
            output = self.s2s(bsrc, btrg, is_train=True)  # output = [batch size, trg len, output dim]
            # print(output.shape)
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.s2s.parameters(), self.config['clip'])
            self.optimizer.step()
    ...
    def run(self):
        for epoch in range(self.config['n_epochs']):
            ...
            if valid_loss < self.best_valid_loss:
                ...
                torch.save(self.s2s.state_dict(), self.config['save_path'])
